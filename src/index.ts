...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'EBI Expression Atlas');
}
...
async function getExperiment(args: Record<string, unknown>): Promise<unknown> {
  ...
  // The accession is the only input on this URL, so a 4xx here is the caller's
  // identifier missing — Atlas answers 400 ("Experiment with accession X could
  // not be found") for unknown and malformed ids alike. The old catch-all also
  // swallowed 5xx/network failures as "experiment not found", which both hid
  // outages and booked caller-side misses as our defect.
  const res = await pwFetch(`${BASE}/json/experiments/${encodeURIComponent(accession)}`, {
    headers: { Accept: 'application/json', 'User-Agent': UA },
  });
  if (res.status === 400 || res.status === 404) {
    return { error: 'user_error', message: experimentMissMessage(accession), accession };
  }
...
// A miss should say what shape Atlas wants and where to go next — "experiment
// not found" told the caller nothing they could act on. The common wrong-ID-
// space cases seen in production: a bare GEO series id (GSE40799), and a valid
// ArrayExpress/GEO accession that Atlas simply never curated (E-GEOD-40799).
function experimentMissMessage(accession: string): string {
  const gse = accession.match(/^GSE(\d+)$/i);
  if (gse) {
    return `Expression Atlas has no experiment "${accession}" — that is a GEO series id. Atlas names GEO-derived experiments E-GEOD-<number> (try "E-GEOD-${gse[1]}"), and only ~4,500 curated experiments are loaded, so not every GEO series is present. Search by keyword/species with search_experiments.`;
  }
  if (!/^E-[A-Z]{3,4}-\d+$/i.test(accession)) {
    return `Expression Atlas has no experiment "${accession}" — Atlas accessions look like "E-MTAB-4045" or "E-GEOD-61857". Search by keyword/species with search_experiments to find one.`;
  }
  return `Expression Atlas has no experiment "${accession}". The accession shape is right, but Atlas loads only ~4,500 curated experiments — a GEO/ArrayExpress accession can exist upstream without being curated into Atlas. Search by keyword/species with search_experiments to find what IS loaded.`;
}
