...
`tools/list` at `https://gateway.pipeworx.io/expression-atlas/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_experiments \
  -H 'Content-Type: application/json' \
  -d '{"query":"liver","species":"Homo sapiens","type":"Baseline","limit":25}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_experiments`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "expression-atlas": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-expression-atlas"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-expression-atlas
```
